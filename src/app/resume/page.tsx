...
export default function ResumePage() {
  ...
  // Internal scroll logic to prevent the whole page/footer from jumping
  useEffect(() => {
        ...
        <div className="flex-none flex items-center justify-between mb-6 border-b border-white/5 pb-4">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[10px] font-black tracking-widest text-gray-500 uppercase">Resume Architect Active</span>
            </div>
            <h1 className="text-2xl font-bold tracking-tighter">Generate <span className="text-gray-500 font-light">Your Resume</span></h1>
          </div>
          <button 
            onClick={() => setMessages([])} 
            className="text-[10px] font-bold text-gray-500 hover:text-white uppercase tracking-[0.2em] transition-colors"
          >
            Reset Session
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto pr-2 space-y-8 custom-scrollbar scroll-smooth">
          {messages.length === 0 && (
            <div className="h-full flex flex-col items-center justify-center opacity-10 text-center">
              ...
              <p className="text-xs font-bold uppercase tracking-[0.5em]">Ready to Build</p>
              <p className="mt-2 text-[10px]">Provide your details to generate a professional profile.</p>
            </div>
          )}
